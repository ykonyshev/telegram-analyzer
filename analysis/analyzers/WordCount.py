...
def word_aggregator(words: pd.Series) -> str:
    return "\n".join(wrap(' '.join(words), max_lines=2))


class WordCountAnalyzer(BaseAnalyzer[Message]):
    _accumulator: defaultdict[tuple[str, MessageType], int] 
    ...
    def parse_text(self, message: Message) -> None:
        for word in message.parse_words():
            self._accumulator[(word, message.message_type)] += 1
    ...
    def _complete_plot(self, save_to: Path) -> None:
        plt.tight_layout()
